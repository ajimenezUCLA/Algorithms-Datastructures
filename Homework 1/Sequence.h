...
// #include <string> // include only when testing strings
#include <iostream>
#include <stdio.h>

// using ItemType = std::string;	// or typedef std::string ItemType;
...
public:
	...
	int find(const ItemType& value) const;								// we pass in value, but we do not modify it,
																		// Let p be the smallest integer such that value == the item at			// therefore we can make this function a const
...
private:
	...
	ItemType m_value[DEFAULT_MAX_ITEMS];		// "items in a sequence are contained in a data member that is an array"
