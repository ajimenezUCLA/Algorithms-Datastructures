...
int subsequence(const Sequence& seq1, const Sequence& seq2)
{
	...
	for (int i = 0; i < seq1.size(); i++)
	{
		...
		ItemType seq2Val;
		seq1.get(i, seq1Val);
		//cerr << "seq1Val = " << seq1Val << endl;
		seq2.get(0, seq2Val);
		//cerr << "seq2Val = " << seq2Val << endl;
		if (seq1Val == seq2Val)
		{
			bool subSeq = true;
			int kStartPos = i;		// has to be created outside of for loop because we return it later
...
void interleave(const Sequence& seq1, const Sequence& seq2, Sequence& result)
{
	// PSEUDO CODE:
	//	new linked list
	// if statements to catch empty list cases
	// intialize length and size variables to zero
	// if seq1 larger, set length to seq1
	// else length is seq2
	// temp datatype for value storage
	// for loop, if get possible, insert value at position of size into result
	//		increment local size variable with insertion
	// update passed list to temp result
	Sequence returnResult;	// temp list inplace of result, to protect against aliasing
	if (seq1.empty())		// easy case
	{
		result = seq2;
	}
	if (seq2.empty())		// easy case
	{
		result = seq1;
	}
	int maxLength = 0;		// length of largest list
	int sizeCount = 0;		// temporary size for temp result
	if (seq1.size() < seq2.size())		// determining which sie is largest
	{
		maxLength = seq2.size();
	}
	else
	...
	ItemType val1;			// place holders for data type
	ItemType val2;
	for (int i = 0; i < maxLength; i++)		// loop for creating new result list
	{
		if (seq1.get(i, val1) == true)		// if can get value, insert it at end of result
		{
			returnResult.insert(sizeCount, val1);
			sizeCount++;
		}
		if (seq2.get(i, val2) == true)	
		{
			returnResult.insert(sizeCount, val2);
			sizeCount++;
		}
	}
	result = returnResult;		// return result
}
